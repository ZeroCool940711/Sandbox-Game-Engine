...
class com.bigworldtech.misc.SimpleDateFormatter
{
	...
	private static var DURATIONS = [
		{	unit:		'milliseconds',
									short: 'ms',	amount: 1 },
		{	unit:		'seconds',	short: 's', 	amount:	1000 },
		{	unit:		'minutes',	short: 'min',	amount: 60 * 1000 },
		{	unit:		'hours',	short: 'hr', 	amount: 60 * 60 * 1000 },
		{	unit: 		'days',		short: 'day',	amount: 24 * 60 * 60 *
																1000 }
	];
	 ...
	 *	@param short		if true, use abbreviated unit names, otherwise, 
	 *						full unit names are used.
	 *	@param delimiter	the delimiter between unit descriptions. If undefined,
	 *						the default, ' ', is used.
	 */
	public static function formatDuration( millis:Number, short:Boolean,
			delimiter:String ):String
	{
		...
		for (var durationIndex:Number = DURATIONS.length - 1;
				durationIndex >= 0 and millis >= 0;
				--durationIndex)
		{
			var duration:Object = DURATIONS[durationIndex];
			// get the remainder
			var divMillis:Number = millis / duration.amount;
			var floor:Number = Math.floor( divMillis );
			if (floor == 0)
			{
				continue;
			}
			var remainder:Number = (divMillis - floor) * duration.amount;

			outUnits.push( floor +
				(short ? duration.short : " " + duration.unit) );
			millis = remainder;
		}
		return outUnits.join( delimiter );
	}
}
